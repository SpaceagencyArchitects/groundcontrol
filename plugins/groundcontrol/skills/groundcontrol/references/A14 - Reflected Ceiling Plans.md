# A14 - Reflected Ceiling Plans    
**A141-A159**  
**A14.##.## (LEVEL.SEQUENCE)**  
  
## REFLECTED CEILING PLANS 1:100
- Doors, only if full height  
- Full height joinery  
- Blinds, curtains, projectors and similar ceiling mounted items  
- All visible services - light fittings, sprinklers, air supply and exhaust smoke detectors, etc.  
- Ceiling mounted signage elements  
- Grid lines  
- Room names, room numbers and finished ceiling levels (FCL)  
- Access panels  
- Change of level indicators  
- Systems and materials codes
- Finishes codes only if not drawn or scheduled in [[A60 - Internal Spaces (Room Layouts)]]  
- Section references to [[A50 - Interface Details]] 
- 'Enlarged area boxes' referring to [[A60 - Internal Spaces (Room Layouts)]] 1:50 reflected ceiling plans. (If a portion of the drawing at 1:100 becomes graphically too heavy due to small rooms and the required amount of description, draw an 'enlarged area reference box', do not show any description within the box at 1:100, and show description items on a 1:50 detail drawing.)  
- Setting-out points  
- Setting-out dimensions  

---
**See also:** [[Tagging of Elements in ArchiCAD]] for ceiling type coding conventions (CLG codes, composite naming).

#groundcontrol #groundcontrol/drawing-series
