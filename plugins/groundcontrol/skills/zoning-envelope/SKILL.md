...
**Requires a planning analysis report** generated by `/planning-analysis-wa`. This skill is a renderer — it does not perform planning calculations.
...
### If a search term is provided (address, lot number, etc.)
...
Search for the most recently modified `planning-analysis-*.md` file in the report directories. If found, confirm with the user. If not found, tell the user:

> No planning analysis report found. Run `/planning-analysis-wa` first, then come back with `/zoning-envelope`.
...
Scale camera distance to the lot's maximum dimension so it works for both small lots (30m) and large suburban / city lots (60m+).
...
| File | Pattern |
|------|---------|
| `./zoning-envelope-48-swanbourne-st.html` | Exact polygon, single lot, flat height cap |
| `./zoning-envelope-multi-lot-example.html` | Multi-lot, 4 scenario toggles |

Use these as the code baseline. Copy utility functions verbatim from the single-lot implementation (cleanest version).
...
- **Units:** WA reports use metres. The `unit` field in the Envelope Data block determines all labels and scaling; imperial is still supported for legacy reports.
